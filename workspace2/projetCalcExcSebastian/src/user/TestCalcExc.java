...
public class TestCalcExc {
	...
	private static void testCalc() {
		...
		try {
			System.out.println("Ce programme ne traite pas les valeurs n�gatives.\nSaisir le premier op�rande :");
			double a = scD.nextDouble();
			System.out.println("Saisir le second op�rande (autre que 0 pour division) :");
			double b = scD.nextDouble();
			System.out.println("Saisir l'op�rateur parmi\n+ add\t- sous\t* mult\t/ div");
			String op = scS.nextLine();
			System.out.println(Utils.choixCalc(a, b, op));
		} catch (NegException ne) {
